...
// ─────────────────────────────────────────────────────────────────────────────
// Templates use single quotes on purpose — do NOT cnvert to backticks
...
const HE_TEMPLATE = 'שלום,\n\nמצ"ב רimport { useState, useEffect, useRef } from "react";
import * as XLSX from "xlsx";
import { db } from "./firebase";
import { doc, onSnapshot, setDoc, getDoc } from "firebase/firestore";
...
const T = {
  en: {
    dir: "ltr", title: "Supply Tracker", subtitle: (c,o,t) => c+" items · "+o+" to order · Total: $"+t,
    search: "Search items...", importBtn: "Import Excel", tplBtn: "Email Template", addBtn: "+ Add Item", sendBtn: "Send Order",
    catAll: "All Items", catNames: { "ציוד קרגו":"Cargo", "ציוד מטבח":"Kitchen", "ציוד משרדי":"Office", "Uline":"Uline", "Amazon":"Amazon" },
    cols: ["Item Name","Per Pack","Order Qty","Unit ($)","Total","Model","Link","Rcvd",""],
    noItems:"No items found", enterPin:"Enter PIN to access", pinPh:"PIN", wrongPin:"Incorrect PIN",
    unlockBtn:"Unlock", editTitle:"Edit Item", addTitle:"Add New Item", catLabel:"Category", newCat:"+ New category",
    itemName:"Item Name", model:"Model Number", perPack:"Units per Pack", orderQty:"Order Quantity",
    costPer:"Cost per Unit ($)", linkLabel:"Product URL", notesLabel:"Notes", cancelBtn:"Cancel", saveBtn:"Save", addItemBtn:"Add",
    tplTitle:"Edit Email Template", tplVars:"Variables:", saveTpl:"Save Template",
    sendTitle:"Send Order Email", recipientLabel:"Recipient Email", subjectLabel:"Subject", defaultSubject:"Supply Order – ",
    previewTitle:"Email Preview", toLabel:"To", subLabel:"Subject", bodyLabel:"Body",
    backBtn:"← Back", sendNow:"Send Now ✉", sending:"Sending...", sentTitle:"Email sent!", sentTo:"Order sent to",
    closeBtn:"Close", errTitle:"Send failed", retryBtn:"Retry", previewBtn:"Preview →",
    openLink:"Open ↗", addLink:"Add URL", rcvdTitle:"Received", footerItems:"items to order", footerTotal:"Total",
    defaultTemplate: EN_TEMPLATE, syncing:"Saving...", syncOk:"Saved ✓", undo:"Undo",
    addSection:"+ Add Section", deleteSection:"Delete Section", confirmDelete:"Delete this section and all its items?",
    sectionName:"Section name",
  },
  he: {
    dir: "rtl", title: "מערכת ניהול ציוד", subtitle: (c,o,t) => c+" פריטים · "+o+" להזמנה · סה\"כ: $"+t,
    search: "חיפוש...", importBtn: "ייבוא Excel", tplBtn: "תבנית מייל", addBtn: "+ הוסף פריט", sendBtn: "שלח הזמנה",
    catAll: "הכל", catNames: { "ציוד קרגו":"ציוד קרגו", "ציוד מטבח":"ציוד מטבח", "ציוד משרדי":"ציוד משרדי", "Uline":"Uline", "Amazon":"Amazon" },
    cols: ["שם מוצר","ב/מארז","כמות","לי' ($)","סה\"כ","Model","קישור","התקבל",""],
    noItems:"אין פריטים", enterPin:"הכנס קוד כניסה", pinPh:"קוד גישה", wrongPin:"קוד שגוי",
    unlockBtn:"כניסה", editTitle:"עריכת פריט", addTitle:"הוספת פריט", catLabel:"קטגוריה", newCat:"+ קטגוריה חדשה",
    itemName:"שם מוצר", model:"מספר דגם", perPack:"יחידות במארז", orderQty:"כמות להזמנה",
    costPer:"עלות ליחידה ($)", linkLabel:"קישור למוצר", notesLabel:"הערות", cancelBtn:"ביטול", saveBtn:"שמור", addItemBtn:"הוסף",
    tplTitle:"עריכת תבנית מייל", tplVars:"משתני תבנית:", saveTpl:"שמור תבנית",
    sendTitle:"שליחת מייל הזמנה", recipientLabel:"כתובת נמען", subjectLabel:"נושא", defaultSubject:"הזמנת ציוד – ",
    previewTitle:"תצוגה מקדימה", toLabel:"נמען", subLabel:"נושא", bodyLabel:"גוף המייל",
    backBtn:"← חזור", sendNow:"שלח עכשיו ✉", sending:"שולח...", sentTitle:"נשלח בהצלחה", sentTo:"נשלח אל",
    closeBtn:"סגור", errTitle:"שגיאה בשליחה", retryBtn:"נסה שוב", previewBtn:"תצוגה מקדימה →",
    openLink:"פתח ↗", addLink:"+ URL", rcvdTitle:"התקבל", footerItems:"להזמנה", footerTotal:"סה\"כ",
    defaultTemplate: HE_TEMPLATE, syncing:"שומר...", syncOk:"נשמר ✓", undo:"בטל",
    addSection:"+ הוסף מדור", deleteSection:"מחק מדור", confirmDelete:"למחוק מדור זה וכל הפריטים שלו?",
    sectionName:"שם המדור",
  },
};

const ACCESS_PIN = "1234";
const DEFAULT_SECTIONS = ["Uline", "Amazon", "ציוד מטבח", "ציוד משרדי"];
const DEFAULT_ITEMS = [
  { id:"d1", category:"Uline",      name:"Uline Industrial Tape CLEAR", unitsPerPack:36, orderQty:0, costPerUnit:1.99,  modelNumber:"S-423",      received:false, link:"https://www.uline.com/Product/Detail/S-423", notes:"" },
  { id:"d2", category:"Uline",      name:"Colored Handwrap - Green",    unitsPerPack:4,  orderQty:0, costPerUnit:26.00, modelNumber:"S-2900G",    received:false, link:"", notes:"" },
  { id:"d3", category:"Amazon",     name:"HP 206A Toner Set",           unitsPerPack:1,  orderQty:0, costPerUnit:329.86,modelNumber:"",           received:false, link:"", notes:"" },
  { id:"d4", category:"ציוד מטבח", name:"Paper Towels",                unitsPerPack:30, orderQty:0, costPerUnit:42.00, modelNumber:"S-7711",     received:false, link:"", notes:"" },
  { id:"d5", category:"ציוד משרדי",name:"BIC Gel Pen Blue",            unitsPerPack:1,  orderQty:0, costPerUnit:1.30,  modelNumber:"S-21758BLU", received:false, link:"", notes:"" },
];
...
// ── COLORS (Professional CRM — light theme) ───────────────────────────────────
const C = {
  bg:       "#f8fafc",
  surface:  "#ffffff",
  border:   "#e2e8f0",
...
function parseSheet(ws, sheetName) {
  ...
  if (!rawRows.length) return { items:[], category: sheetName };

  let headerRowIdx = -1;
  let supplier = sheetName;

  for (let i = 0; i < rawRows.length; i++) {
    const row = rawRows[i];
    // Row 2 usually has supplier name (ULINE, AMAZON)
    if (i === 1 && row[0]) supplier = String(row[0]).trim();
    if (row.some(cell => String(cell).trim() === "מוצר")) { headerRowIdx = i; break; }
  }

  if (headerRowIdx === -1) return { items:[], category: supplier };
  ...
  if (nameIdx === -1) return { items:[], category: supplier };

  const cleanNum  = (v) => parseFloat(String(v).replace(/[$,\s=]/g,"")) || 0;
  const cleanBool = (v) => v===true || ["כן","yes","true","1","v","✓"].includes(String(v).toLowerCase().trim());

  // Normalize supplier name: ULINE→Uline, AMAZON→Amazon
  const normalize = (s) => s.charAt(0).toUpperCase() + s.slice(1).toLowerCase();
  const category = normalize(supplier);
...
const store = {
  get: async (key) => {
    if (typeof window !== "undefined" && window.storage && typeof window.storage.get === "function") {
      try { const r = await window.storage.get(key); return r ? r.value : null; } catch {}
    }
...
// ── STYLES ────────────────────────────────────────────────────────────────────
const S = {
  input: { width:"100%", background:C.surface, border:"1px solid "+C.border, color:C.text, borderRadius:6, padding:"8px 10px", fontSize:13, outline:"none", boxSizing:"border-box", fontFamily:"inherit" },
  btn: (bg, color, border) => ({ background:bg, border:"1px solid "+(border||bg), color, borderRadius:6, padding:"8px 14px", fontSize:13, fontWeight:500, cursor:"pointer", fontFamily:"inherit", transition:"all .15s" }),
  icon: { background:"none", border:"none", cursor:"pointer", fontSize:14, padding:"3px 5px", borderRadius:4, transition:"all .15s", color:C.textMuted },
  stepBtn: { width:22, height:22, borderRadius:4, background:C.surface, border:"1px solid "+C.border, color:C.textSub, cursor:"pointer", fontSize:15, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:600, padding:0 },
};
...
function QtyStep({ value, onChange }) {
  return (
    <div style={{ display:"flex", alignItems:"center", gap:5, justifyContent:"center" }}>
      ...
      <span style={{ minWidth:28, textAlign:"center", fontFamily:"monospace", fontWeight:600, fontSize:13, color: value>0 ? C.accent : C.textMuted }}>{value}</span>
...
function InlineEdit({ value, type="text", placeholder="—", onCommit }) {
  ...
  if (editing) return (
    <input autoFocus type={type} value={draft} onChange={e => setDraft(e.target.value)}
      ...
      style={{ width:"100%", background:C.accentBg, border:"1px solid "+C.accent, color:C.text, borderRadius:4, padding:"3px 7px", fontSize:12, outline:"none", textAlign:"center", boxSizing:"border-box", direction:"ltr" }}
    />
  );
  return (
    <span title="Click to edit" onClick={() => { setDraft(value); setEditing(true); }}
      onMouseEnter={e => { e.currentTarget.style.background=C.accentBg; e.currentTarget.style.borderRadius="4px"; }}
      onMouseLeave={e => { e.currentTarget.style.background="transparent"; }}
      style={{ cursor:"text", color:value?C.text:C.textMuted, fontSize:12, display:"block", textAlign:"center", padding:"3px 5px", direction:"ltr" }}>
      {value || <span style={{ color:C.textMuted, fontStyle:"italic" }}>{placeholder}</span>}
      {value && <span style={{ opacity:0.3, fontSize:8, marginLeft:2 }}>✎</span>}
    </span>
  );
}
...
function PinScreen({ t, onUnlock }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:20, direction:t.dir }}>
      <div style={{ background:C.surface, borderRadius:16, padding:40, boxShadow:C.shadowLg, display:"flex", flexDirection:"column", alignItems:"center", gap:16, minWidth:300 }}>
        <div style={{ fontSize:36 }}>✈</div>
        <div style={{ fontSize:16, fontWeight:700, color:C.text }}>{t.enterPin}</div>
        <input type="password" value={pin} autoFocus placeholder={t.pinPh}
          ...
          style={{ ...S.input, textAlign:"center", letterSpacing:"0.3em", direction:"ltr", fontFamily:"monospace", fontSize:20, padding:"12px 16px", border:"1px solid "+(err?C.danger:C.border) }}
        />
        {err && <div style={{ color:C.danger, fontSize:12 }}>{t.wrongPin}</div>}
        <button onClick={submit} style={{ ...S.btn(C.accent,"#fff"), width:"100%", padding:"11px 0", fontSize:14, fontWeight:600 }}>{t.unlockBtn}</button>
      </div>
    </div>
  );
}
...
// ── ITEM MODAL ────────────────────────────────────────────────────────────────
function ItemModal({ t, item, sections, onSave, onClose }) {
  const blank = { id:uid(), category:sections[0]||"Uline", name:"", unitsPerPack:1, orderQty:0, costPerUnit:0, modelNumber:"", received:false, link:"", notes:"" };
  ...
  const fields = [
    {key:"name",label:t.itemName,type:"text"},{key:"modelNumber",label:t.model,type:"text"},
    {key:"unitsPerPack",label:t.perPack,type:"number"},{key:"orderQty",label:t.orderQty,type:"number"},
    {key:"costPerUnit",label:t.costPer,type:"number"},{key:"link",label:t.linkLabel,type:"text"},{key:"notes",label:t.notesLabel,type:"text"},
  ];
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(15,23,42,0.4)", zIndex:50, display:"flex", alignItems:"center", justifyContent:"center" }} onClick={onClose}>
      <div style={{ background:C.surface, borderRadius:14, padding:28, width:440, maxHeight:"85vh", overflowY:"auto", boxShadow:C.shadowLg, direction:t.dir }} onClick={e=>e.stopPropagation()}>
        <div style={{ fontSize:15, fontWeight:700, marginBottom:20, color:C.text }}>{item?t.editTitle:t.addTitle}</div>
        <div style={{ marginBottom:14 }}>
          <div style={{ fontSize:11, color:C.textSub, marginBottom:5, fontWeight:500, textTransform:"uppercase", letterSpacing:"0.05em" }}>{t.catLabel}</div>
          {customCat ? (
            ...
            <select value={form.category} style={{...S.input,direction:t.dir,cursor:"pointer"}} onChange={e=>{if(e.target.value==="__new__"){set("category","");setCustomCat(true);}else set("category",e.target.value);}} onFocus={ff} onBlur={fb}>
              {sections.map(c=><option key={c} value={c}>{t.catNames[c]||c}</option>)}
        ...
        {fields.map(f=>(
          <div key={f.key} style={{marginBottom:14}}>
            <div style={{fontSize:11,color:C.textSub,marginBottom:5,fontWeight:500,textTransform:"uppercase",letterSpacing:"0.05em"}}>{f.label}</div>
        ...
        <div style={{display:"flex",gap:8,marginTop:24}}>
          <button style={{...S.btn(C.surface,C.textSub,C.border),flex:1}} onClick={onClose}>{t.cancelBtn}</button>
          <button style={{...S.btn(C.accent,"#fff"),flex:2}} onClick={()=>{onSave({...form});onClose();}}>{item?t.saveBtn:t.addItemBtn}</button>
        </div>
      </div>
    </div>
  );
...
function TemplateEditor({ t, template, onSave, onClose }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.4)",zIndex:50,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={onClose}>
      <div style={{background:C.surface,borderRadius:14,padding:28,width:520,boxShadow:C.shadowLg,direction:t.dir}} onClick={e=>e.stopPropagation()}>
        <div style={{fontSize:15,fontWeight:700,marginBottom:8,color:C.text}}>{t.tplTitle}</div>
        <div style={{fontSize:12,color:C.textSub,marginBottom:16,lineHeight:1.8}}>
          {t.tplVars}&nbsp;
          {["{{date}}","{{items}}","{{totalCost}}"].map(v=>(
            <code key={v} style={{background:C.accentBg,padding:"1px 6px",borderRadius:4,color:C.accent,margin:"0 3px",fontSize:11}}>{v}</code>
          ))}
        </div>
        <textarea value={val} rows={12} onChange={e=>setVal(e.target.value)}
          onFocus={e=>{e.target.style.borderColor=C.accent;}} onBlur={e=>{e.target.style.borderColor=C.border;}}
          style={{width:"100%",background:C.bg,border:"1px solid "+C.border,color:C.text,borderRadius:8,padding:14,fontSize:13,outline:"none",resize:"vertical",lineHeight:1.8,boxSizing:"border-box",direction:t.dir,fontFamily:"inherit"}}
        />
        <div style={{display:"flex",gap:8,marginTop:16}}>
          <button style={{...S.btn(C.surface,C.textSub,C.border),flex:1}} onClick={onClose}>{t.cancelBtn}</button>
          <button style={{...S.btn(C.accent,"#fff"),flex:2}} onClick={()=>{onSave(val);onClose();}}>{t.saveTpl}</button>
        </div>
      </div>
    </div>
  );
...
function SendFlow({ t, orderItems, template, onClose }) {
  ...
  const buildBody = () => {
    ...
    const lines = orderItems.map(i=>{
      let line="• "+i.name; if(i.modelNumber) line+=" ["+i.modelNumber+"]";
      line+="\n  Qty: "+i.orderQty+" | Cost: $"+(i.orderQty*i.costPerUnit).toFixed(2);
      if(i.link) line+="\n  Link: "+i.link; return line;
    }).join("\n\n");
    return template.replace("{{date}}",date).replace("{{items}}",lines).replace("{{totalCost}}",total);
  };

  const doSend = async () => {
    setStep("sending");
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:500,system:'Send the email using Gmail MCP. Reply only: {"sent":true}',messages:[{role:"user",content:"Send email:\nTo: "+preview.to+"\nSubject: "+preview.subject+"\nBody:\n"+preview.body}],mcp_servers:[{type:"url",url:"https://gmailmcp.googleapis.com/mcp/v1",name:"gmail-mcp"}]})});
      await res.json(); setStep("sent");
    } catch { setStep("error"); }
  };

  const overlay = (
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.4)",zIndex:50,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={step==="sending"?undefined:onClose}>
      <div style={{background:C.surface,borderRadius:14,padding:28,width:480,maxHeight:"85vh",overflowY:"auto",boxShadow:C.shadowLg,direction:t.dir}} onClick={e=>e.stopPropagation()}>

        {step==="input" && <>
          <div style={{fontSize:15,fontWeight:700,marginBottom:4,color:C.text}}>{t.sendTitle}</div>
          <div style={{fontSize:12,color:C.textSub,marginBottom:16}}>{orderItems.length} items · ${total}</div>
          <div style={{background:C.bg,borderRadius:8,border:"1px solid "+C.border,padding:"10px 14px",marginBottom:18,maxHeight:140,overflowY:"auto"}}>
            {orderItems.map(i=>(
              <div key={i.id} style={{display:"flex",justifyContent:"space-between",fontSize:12,color:C.textSub,padding:"4px 0",borderBottom:"1px solid "+C.border}}>
                <span>{i.name}</span>
                <span style={{color:C.accent,fontFamily:"monospace",fontWeight:600}}>{"x"+i.orderQty+" = $"+(i.orderQty*i.costPerUnit).toFixed(2)}</span>
              </div>
            ))}
          </div>
          {[{l:t.recipientLabel,v:to,s:setTo,d:"ltr"},{l:t.subjectLabel,v:subject,s:setSubject,d:t.dir}].map(f=>(
            <div key={f.l} style={{marginBottom:14}}>
              <div style={{fontSize:11,color:C.textSub,marginBottom:5,fontWeight:500,textTransform:"uppercase",letterSpacing:"0.05em"}}>{f.l}</div>
              <input value={f.v} onChange={e=>f.s(e.target.value)} style={{...S.input,direction:f.d}} onFocus={e=>{e.target.style.borderColor=C.accent;}} onBlur={e=>{e.target.style.borderColor=C.border;}} />
            </div>
          ))}
          <div style={{display:"flex",gap:8,marginTop:20}}>
            <button style={{...S.btn(C.surface,C.textSub,C.border),flex:1}} onClick={onClose}>{t.cancelBtn}</button>
            <button disabled={!to.trim()} onClick={()=>{setPreview({to,subject,body:buildBody()});setStep("preview");}}
              style={{...S.btn(to.trim()?C.accent:C.border,to.trim()?"#fff":C.textMuted),flex:2,cursor:to.trim()?"pointer":"not-allowed"}}>{t.previewBtn}</button>
          </div>
        </>}

        {step==="preview"&&preview&&<>
          <div style={{fontSize:15,fontWeight:700,marginBottom:20,color:C.text}}>{t.previewTitle}</div>
          {[{l:t.toLabel,v:preview.to,d:"ltr"},{l:t.subLabel,v:preview.subject,d:t.dir}].map(f=>(
            <div key={f.l} style={{marginBottom:12}}>
              <div style={{fontSize:10,color:C.textMuted,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:4}}>{f.l}</div>
              <div style={{background:C.bg,borderRadius:6,border:"1px solid "+C.border,padding:"8px 12px",fontSize:13,color:C.text,direction:f.d}}>{f.v}</div>
            </div>
          ))}
          <div style={{marginBottom:20}}>
            <div style={{fontSize:10,color:C.textMuted,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:4}}>{t.bodyLabel}</div>
            <div style={{background:C.bg,borderRadius:6,border:"1px solid "+C.border,padding:"12px 14px",fontSize:12,color:C.textSub,whiteSpace:"pre-wrap",lineHeight:1.8,maxHeight:200,overflowY:"auto",direction:t.dir}}>{preview.body}</div>
          </div>
          <div style={{display:"flex",gap:8}}>
            <button style={{...S.btn(C.surface,C.textSub,C.border),flex:1}} onClick={()=>setStep("input")}>{t.backBtn}</button>
            <button style={{...S.btn(C.accent,"#fff"),flex:2}} onClick={doSend}>{t.sendNow}</button>
          </div>
        </>}

        {step==="sending"&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:16,padding:"28px 0"}}>
          <style>{"@keyframes spin{to{transform:rotate(360deg)}}"}</style>
          <div style={{width:32,height:32,border:"3px solid "+C.border,borderTop:"3px solid "+C.accent,borderRadius:"50%",animation:"spin .8s linear infinite"}}/>
          <div style={{color:C.textSub,fontSize:13}}>{t.sending}</div>
        </div>}

        {step==="sent"&&<div style={{textAlign:"center",padding:"16px 0"}}>
          <div style={{fontSize:40,marginBottom:12}}>✅</div>
          <div style={{fontSize:15,fontWeight:700,marginBottom:6,color:C.text}}>{t.sentTitle}</div>
          <div style={{fontSize:12,color:C.textSub,marginBottom:24}}>{t.sentTo} {preview&&preview.to}</div>
          <button style={{...S.btn(C.accent,"#fff"),padding:"9px 32px"}} onClick={onClose}>{t.closeBtn}</button>
        </div>}

        {step==="error"&&<div style={{textAlign:"center",padding:"16px 0"}}>
          <div style={{fontSize:40,marginBottom:12}}>❌</div>
          <div style={{fontSize:15,fontWeight:700,marginBottom:24,color:C.text}}>{t.errTitle}</div>
          <div style={{display:"flex",gap:8,justifyContent:"center"}}>
            <button style={{...S.btn(C.surface,C.textSub,C.border),padding:"9px 20px"}} onClick={()=>setStep("preview")}>{t.backBtn}</button>
            <button style={{...S.btn(C.accent,"#fff"),padding:"9px 20px"}} onClick={doSend}>{t.retryBtn}</button>
          </div>
        </div>}
      </div>
    </div>
...
export default function App() {
  ...
  useEffect(() => { try { if(sessionStorage.getItem("cargo-unlocked")==="1") setUnlocked(true); }catch{} }, []);
  const handleUnlock = () => { try{sessionStorage.setItem("cargo-unlocked","1");}catch{} setUnlocked(true); };

  useEffect(() => {
    if (!unlocked) return;
    const unsub = onSnapshot(ITEMS_DOC, (snap) => {
      if (snap.exists()) {
        const data = snap.data();
        setItems(data.list || DEFAULT_ITEMS);
        if (data.sections) setSections(data.sections);
      } else {
        setDoc(ITEMS_DOC, { list:DEFAULT_ITEMS, sections:DEFAULT_SECTIONS });
    ...
    getDoc(SETTINGS_DOC).then((snap) => {
      if (snap.exists()) {
        ...
        setLang(l);
        setTemplate(data.template||T[l].defaultTemplate);
      } else setTemplate(T["en"].defaultTemplate);
    });
    return () => unsub();
  }, [unlocked]);

  const saveItems = (newItems, newSections) => {
    setSyncStatus("syncing");
    if (saveTimer.current) clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(async () => {
      await setDoc(ITEMS_DOC, { list:newItems, sections: newSections||sections });
      setSyncStatus("ok");
    }, 800);
  };

  const setAndSave = (newItems, newSections) => {
    // Push current state to history before changing
    setHistory(h => [...h.slice(-MAX_HISTORY), items]);
    setItems(newItems);
    if (newSections) setSections(newSections);
    saveItems(newItems, newSections||sections);
  };
  ...
  const addSection = () => {
    ...
    saveItems(items, newSections);
  };

  const deleteSection = (cat) => {
    if (!window.confirm(t.confirmDelete)) return;
    const newItems    = items.filter(i=>i.category!==cat);
    const newSections = sections.filter(s=>s!==cat);
    setAndSave(newItems, newSections);
    if (activeCat===cat) setActiveCat("__all__");
  };
  ...
  const switchLang = (l) => {
    ...
    setTemplate(tpl);
    setDoc(SETTINGS_DOC, { lang:l, template:tpl });
  };
  ...
  const handleExcel = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        let allItems=[]; let newSections=[...sections];
        for (const sheetName of wb.SheetNames) {
          const ws = wb.Sheets[sheetName];
          const { items:sheetItems, category } = parseSheet(ws, sheetName);
          allItems = [...allItems, ...sheetItems];
          if (!newSections.includes(category) && sheetItems.length>0) newSections.push(category);
        }
        if (allItems.length>0) {
          setAndSave(allItems, newSections);
          alert("Imported "+allItems.length+" items from "+wb.SheetNames.length+" sheets!");
        } else alert("No items found. Make sure Excel has a column named מוצר");
  ...
  const allCats    = [...new Set([...sections, ...items.map(i=>i.category)])];
  const filtered   = items.filter(i=>(activeCat==="__all__"||i.category===activeCat)&&(!search||i.name.toLowerCase().includes(search.toLowerCase())||(i.modelNumber||"").toLowerCase().includes(search.toLowerCase())));
  ...
  const COLS = "2.2fr 0.65fr 1fr 0.7fr 0.75fr 0.85fr 0.65fr 0.5fr 60px";

  return (
    <div style={{ minHeight:"100vh", background:C.bg, color:C.text, fontFamily:"-apple-system,'Segoe UI',sans-serif", direction:t.dir }}>
      <style>{"*{box-sizing:border-box}::-webkit-scrollbar{width:5px;height:5px}::-webkit-scrollbar-track{background:"+C.bg+"}::-webkit-scrollbar-thumb{background:"+C.border+";border-radius:3px}"}</style>

      {/* ── HEADER ── */}
      <div style={{ background:C.surface, borderBottom:"1px solid "+C.border, padding:"14px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12, boxShadow:C.shadow }}>
        <div>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <span style={{ fontSize:11, color:syncStatus==="syncing"?C.warning:C.success, fontWeight:500 }}>
              {syncStatus==="syncing"?t.syncing:t.syncOk}
            </span>
          </div>
          <div style={{ fontSize:11, color:C.textSub, marginTop:3, marginLeft:42 }}>{t.subtitle(items.length,orderItems.length,orderTotal)}</div>
        </div>
        ...
        <div style={{ display:"flex", gap:7, flexWrap:"wrap", alignItems:"center" }}>
          ...
          <div style={{ display:"flex", background:C.bg, border:"1px solid "+C.border, borderRadius:6, overflow:"hidden" }}>
            {["en","he"].map(l=>(
              <button key={l} onClick={()=>switchLang(l)}
                style={{ padding:"6px 12px", border:"none", background:lang===l?C.accent:"transparent", color:lang===l?"#fff":C.textSub, fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>
                {l==="en"?"EN":"עב"}
              </button>
            ))}
          </div>
          ...
          {/* Undo */}
          <button onClick={handleUndo} disabled={!history.length} title={t.undo}
            style={{ ...S.btn(C.surface,history.length?C.textSub:C.textMuted,C.border), padding:"7px 12px", fontSize:12 }}>
            ↩ {t.undo}
          </button>

          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder={"🔍 "+t.search}
            style={{ ...S.input, width:160, padding:"7px 12px" }}
            onFocus={e=>{e.target.style.borderColor=C.accent;}} onBlur={e=>{e.target.style.borderColor=C.border;}}
          />
          <input type="file" accept=".xlsx,.xls,.csv" ref={fileRef} onChange={handleExcel} style={{display:"none"}} />
          <button style={S.btn(C.surface,C.textSub,C.border)} onClick={()=>fileRef.current.click()}>{t.importBtn}</button>
          <button style={S.btn(C.surface,C.textSub,C.border)} onClick={()=>setShowTpl(true)}>{t.tplBtn}</button>
          <button style={{...S.btn(C.accentBg,C.accent,C.accent)}} onClick={()=>setShowAdd(true)}>{t.addBtn}</button>
          <button disabled={orderItems.length===0} onClick={()=>setShowEmail(true)}
            style={{...S.btn(orderItems.length?C.accent:C.border,orderItems.length?"#fff":C.textMuted), cursor:orderItems.length?"pointer":"not-allowed", fontWeight:600}}>
      ...
      <div style={{ display:"flex", minHeight:"calc(100vh - 70px)" }}>
        ...
        {/* Sidebar */}
        <div style={{ width:200, background:C.surface, borderRight:"1px solid "+C.border, padding:"16px 0", flexShrink:0 }}>
          {[{id:"__all__",label:t.catAll,count:items.length},...allCats.map(c=>({id:c,label:t.catNames[c]||c,count:items.filter(i=>i.category===c).length}))].map(cat=>(
            <div key={cat.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 8px" }}>
              <button onClick={()=>setActiveCat(cat.id)}
                style={{ flex:1, textAlign:t.dir==="rtl"?"right":"left", padding:"8px 10px", borderRadius:6, border:"none", background:activeCat===cat.id?C.accentBg:"transparent", color:activeCat===cat.id?C.accent:C.textSub, fontSize:13, fontWeight:activeCat===cat.id?600:400, cursor:"pointer", fontFamily:"inherit", transition:"all .15s" }}>
                {cat.label}
                <span style={{ marginLeft:6, fontSize:11, opacity:0.6 }}>({cat.count})</span>
              </button>
              {cat.id!=="__all__" && (
                <button onClick={()=>deleteSection(cat.id)} title={t.deleteSection}
                  style={{ ...S.icon, fontSize:12, opacity:0.4 }}
                  onMouseEnter={e=>{e.currentTarget.style.color=C.danger;e.currentTarget.style.opacity="1";}}
                  onMouseLeave={e=>{e.currentTarget.style.color=C.textMuted;e.currentTarget.style.opacity="0.4";}}>
                  ✕
                </button>
              )}
            </div>
          ...
          <button onClick={addSection}
            style={{ width:"calc(100% - 16px)", margin:"8px 8px 0", padding:"8px 10px", borderRadius:6, border:"1px dashed "+C.border, background:"transparent", color:C.textMuted, fontSize:12, cursor:"pointer", fontFamily:"inherit", textAlign:"center" }}
        ...
        <div style={{ flex:1, overflow:"auto" }}>
          {/* Column headers */}
          <div style={{ display:"grid", gridTemplateColumns:COLS, gap:8, padding:"10px 20px", borderBottom:"1px solid "+C.border, background:C.bg, position:"sticky", top:0, zIndex:10 }}>
            {t.cols.map((h,i)=>(
              <span key={i} style={{ fontSize:10, color:C.textMuted, letterSpacing:"0.08em", textTransform:"uppercase", fontWeight:600, textAlign:i===0?(t.dir==="rtl"?"right":"left"):"center" }}>{h}</span>
            ))}
          </div>

          {/* Rows */}
          {filtered.length===0 && <div style={{textAlign:"center",padding:48,color:C.textMuted,fontSize:13}}>{t.noItems}</div>}
          {filtered.map(item=>(
            <div key={item.id}
              style={{ display:"grid", gridTemplateColumns:COLS, gap:8, padding:"9px 20px", borderBottom:"1px solid "+C.border, alignItems:"center", background:C.surface, transition:"background .1s" }}
              onMouseEnter={e=>{e.currentTarget.style.background=C.bg;}}
              onMouseLeave={e=>{e.currentTarget.style.background=C.surface;}}>

              <div style={{display:"flex",alignItems:"center",gap:7,overflow:"hidden"}}>
                {item.orderQty>0&&!item.received&&<span style={{width:6,height:6,borderRadius:"50%",background:C.accent,flexShrink:0}}/>}
                <span title={item.name} onDoubleClick={()=>setEditItem(item)}
                  style={{fontSize:13,color:item.received?C.textMuted:C.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",textDecoration:item.received?"line-through":"none"}}>
                  {item.name}
                </span>
              </div>
              ...
              <div style={{textAlign:"center",fontFamily:"monospace",fontSize:12,fontWeight:600,color:item.orderQty>0&&!item.received?C.accent:C.textMuted}}>
              ...
              <div style={{textAlign:"center"}}>
                {item.link
                  ? <a href={item.link} target="_blank" rel="noopener noreferrer" style={{color:C.accent,fontSize:12,textDecoration:"none",fontWeight:500}}>{t.openLink}</a>
              ...
              <div style={{display:"flex",justifyContent:"center"}}>
                <button onClick={()=>patchItem(item.id,{received:!item.received})}
                  style={{width:20,height:20,borderRadius:4,border:"2px solid "+(item.received?C.success:C.border),background:item.received?C.successBg:"transparent",cursor:"pointer",fontSize:11,display:"flex",alignItems:"center",justifyContent:"center",color:C.success}}>
                  {item.received?"✓":""}
                </button>
              </div>
              ...
              <div style={{display:"flex",gap:3,justifyContent:"center"}}>
                <button style={{...S.icon}} onClick={()=>setEditItem(item)}
                  ...
                  onMouseLeave={e=>{e.currentTarget.style.color=C.textMuted;e.currentTarget.style.background="transparent";}}>✎</button>
          ...
          {/* Footer */}
          {orderItems.length>0&&(
            <div style={{padding:"12px 20px",borderTop:"1px solid "+C.border,display:"flex",justifyContent:"flex-end",gap:20,fontSize:13,background:C.surface}}>
              <span style={{color:C.textSub}}>{orderItems.length} {t.footerItems}</span>
              <span style={{color:C.accent,fontWeight:700,fontFamily:"monospace"}}>{t.footerTotal}: ${orderTotal}</span>
            </div>
          )}
        </div>
      </div>

      {showAdd   && <ItemModal t={t} sections={allCats} onSave={upsertItem} onClose={()=>setShowAdd(false)} />}
      {editItem  && <ItemModal t={t} item={editItem} sections={allCats} onSave={upsertItem} onClose={()=>setEditItem(null)} />}
      {showTpl   && <TemplateEditor t={t} template={template} onSave={tpl=>{setTemplate(tpl);setDoc(SETTINGS_DOC,{lang,template:tpl});}} onClose={()=>setShowTpl(false)} />}
      {showEmail && <SendFlow t={t} orderItems={orderItems} template={template} onClose={()=>setShowEmail(false)} />}
    </div>
  );
}שימת הזמנה לחידוש ציוד – {{date}}\n\n{{items}}\n\nסה"כ עלות משוערת: ${{totalCost}}\n\nאנא אשר קבלת ההזמנה.\n\nתודה';
