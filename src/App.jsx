...
const T = {
  en: {
    ...
    subtitle: (count, order, total) =>
      `${count} items · ${order} to order · Total: $${total}`,
    search: "Search...",
    importBtn: "⬆ Excel",
    tplBtn: "✉ Template",
    addBtn: "+ Item",
  ...
  he: {
    ...
    subtitle: (count, order, total) =>
      `${count} \u05E4\u05E8\u05D9\u05D8\u05D9\u05DD \u00B7 ${order} \u05DC\u05D4\u05D6\u05DE\u05E0\u05D4 \u00B7 \u05E1\u05D4"\u05DB: $${total}`,
    search: "\u05D7\u05D9\u05E4\u05D5\u05E9...",
    importBtn: "\u2B06 Excel",
    tplBtn: "\u2709 \u05EA\u05D1\u05E0\u05D9\u05EA",
    addBtn: "+ \u05E4\u05E8\u05D9\u05D8",
...
const CAT_CARGO  = "\u05E6\u05D9\u05D5\u05D3 \u05E7\u05E8\u05D2\u05D5";
const CAT_KITCHEN= "\u05E6\u05D9\u05D5\u05D3 \u05DE\u05D8\u05D1\u05D7";
const CAT_OFFICE = "\u05E6\u05D9\u05D5\u05D3 \u05DE\u05E9\u05E8\u05D3\u05D9";
const ALL_CATS   = [CAT_CARGO, CAT_KITCHEN, CAT_OFFICE];
...
const DEFAULT_ITEMS = [
  { id:"d1", category:CAT_CARGO,  name:"Uline Industrial Tape CLEAR", unitsPerPack:36, orderQty:0, costPerUnit:1.99,  modelNumber:"S-423",      received:false, link:"https://www.uline.com/Product/Detail/S-423", notes:"" },
  { id:"d2", category:CAT_CARGO,  name:"Colored Handwrap - Green",    unitsPerPack:4,  orderQty:0, costPerUnit:26.00, modelNumber:"S-2900G",    received:false, link:"", notes:"" },
  { id:"d3", category:CAT_KITCHEN,name:"Paper Towels",                unitsPerPack:30, orderQty:0, costPerUnit:42.00, modelNumber:"S-7711",     received:false, link:"", notes:"" },
  { id:"d4", category:CAT_OFFICE, name:"BIC Gel Pen - Medium Blue",   unitsPerPack:1,  orderQty:0, costPerUnit:1.30,  modelNumber:"S-21758BLU", received:false, link:"", notes:"" },
];
...
function parseSheet(ws, sheetName) {
  ...
  let headerRowIdx = -1;
  let category = sheetName || CAT_CARGO;
...
const S = {
  ...
  primary: {
    background:"#f59e0b", border:"none", color:"#000", borderRadius:6,
  ...
  icon: {
    ...
    padding:"2px 5px", borderRadius:3, transition:"color 0.15s",
  },
  stepBtn: {
...
function QtyStep({ value, onChange }) {
  return (
    <div style={{ display:"flex", alignItems:"center", gap:4, justifyContent:"center" }}>
      ...
      <span style={{ minWidth:28, textAlign:"center", fontFamily:"monospace", fontWeight:700, fontSize:13, color:value > 0 ? "#fbbf24" : "#52525b" }}>
        {value}
      </span>
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#09090b", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:20, direction:t.dir }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:10 }}>
        <input
          type="password" value={pin} autoFocus placeholder={t.pinPh}
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.78)", zIndex:50, display:"flex", alignItems:"center", justifyContent:"center" }} onClick={onClose}>
      <div style={{ background:"#18181b", border:"1px solid #3f3f46", borderRadius:14, padding:28, width:420, maxHeight:"85vh", overflowY:"auto", boxShadow:"0 24px 64px rgba(0,0,0,0.6)", direction:t.dir }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ marginBottom:14 }}>
          ...
          {customCat ? (
            <input
              value={form.category} autoFocus
              style={{ ...S.input, direction:t.dir }}
              onChange={e => set("category", e.target.value)}
              onFocus={ff}
              onBlur={e => { fb(e); if (!form.category) setCustomCat(false); }}
            />
          ) : (
            <select
              value={form.category}
              style={{ ...S.input, direction:t.dir, cursor:"pointer" }}
              onChange={e => {
                if (e.target.value === "__new__") { set("category", ""); setCustomCat(true); }
...
function SendFlow({ t, orderItems, template, onClose }) {
  ...
  const buildBody = () => {
    const date = new Date().toLocaleDateString(t.dir === "rtl" ? "he-IL" : "en-US", {
      ...
      <div style={{ background:"#18181b", border:"1px solid #3f3f46", borderRadius:14, padding:28, width:480, maxHeight:"85vh", overflowY:"auto", boxShadow:"0 24px 64px rgba(0,0,0,0.6)", direction:t.dir }} onClick={e => e.stopPropagation()}>
        ...
        {step === "input" && (
          <>
            ...
            {[
              { l:t.recipientLabel, v:to,      s:setTo,      d:"ltr"  },
              { l:t.subjectLabel,   v:subject,  s:setSubject, d:t.dir  },
            ].map(f => (
              <div key={f.l} style={{ marginBottom:14 }}>
                <div style={{ fontSize:11, color:"#71717a", marginBottom:5 }}>{f.l}</div>
                <input
                  value={f.v} onChange={e => f.s(e.target.value)}
                  style={{ ...S.input, direction:f.d }}
                  onFocus={e => { e.target.style.borderColor = "#f59e0b"; }}
                  onBlur={e => { e.target.style.borderColor = "#3f3f46"; }}
                />
              </div>
        ...
        {step === "preview" && preview && (
          <>
            ...
            {[
              { l:t.toLabel,  v:preview.to,      d:"ltr"  },
              { l:t.subLabel, v:preview.subject,  d:t.dir  },
            ].map(f => (
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubItems = onSnapshot(ITEMS_DOC, snap => {
      if (snap.exists()) {
        setItems(snap.data().list || DEFAULT_ITEMS);
      } else {
  ...
  const saveSettings = (newLang, newTemplate) =>
    setDoc(SETTINGS_DOC, { lang: newLang, template: newTemplate });
  ...
  const switchLang = l => {
    setLang(l);
    const tpl = (template === T["en"].defaultTemplate || template === T["he"].defaultTemplate)
      ? T[l].defaultTemplate
      : template;
    setTemplate(tpl);
    saveSettings(l, tpl);
    ...
    reader.onload = ev => {
      try {
        ...
        for (const sheetName of wb.SheetNames) {
          allItems = [...allItems, ...parseSheet(wb.Sheets[sheetName], sheetName)];
        }
        if (allItems.length > 0) {
          setAndSave(allItems);
          alert(`Imported ${allItems.length} items from ${wb.SheetNames.length} sheet(s)`);
        } else {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#09090b", color:"#e4e4e7", fontFamily:"'Heebo','Segoe UI',sans-serif", direction:t.dir }}>
      <style>
        {"@import url('https://fonts.googleapis.com/css2?family=Heebo:wght@400;500;600;700&display=swap');::-webkit-scrollbar{width:5px;height:5px}::-webkit-scrollbar-track{background:#18181b}::-webkit-scrollbar-thumb{background:#3f3f46;border-radius:3px}"}
      </style>
      ...
      <div style={{ display:"flex", gap:4, padding:"10px 24px", borderBottom:"1px solid #1c1c1f", overflowX:"auto" }}>
        {[
          { id:"__all__", label:t.catAll, count:items.length },
          ...allCats.map(c => ({ id:c, label:t.catNames[c] || c, count:items.filter(i => i.category === c).length })),
        ].map(cat => (
          <button
      ...
      <div style={{ display:"grid", gridTemplateColumns:COLS, gap:8, padding:"7px 20px", borderBottom:"1px solid #1c1c1f" }}>
        {t.cols.map((h, i) => (
          <span key={i} style={{ fontSize:10, color:"#52525b", letterSpacing:"0.06em", textTransform:"uppercase", fontWeight:600, textAlign:i === 0 ? (t.dir === "rtl" ? "right" : "left") : "center" }}>
            {h}
          </span>
        ))}
      </div>
        ...
        {filtered.map(item => (
          ...
          >
            ...
            <div style={{ display:"flex", gap:3, justifyContent:"center" }}>
              <button
                style={{ ...S.icon, color:"#52525b" }}
                onClick={() => setEditItem(item)}
                onMouseEnter={e => { e.currentTarget.style.color = "#e4e4e7"; }}
                onMouseLeave={e => { e.currentTarget.style.color = "#52525b"; }}
              >✎</button>
              <button
                style={{ ...S.icon, color:"#3f3f46" }}
