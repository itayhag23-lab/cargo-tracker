...
// ─────────────────────────────────────────────────────────────────────────────
// IMPORTANT: Default templates use single quotes on purpose.
// Do NOT convert them to backticks — ${{x}} would break as a JS template expr.
// ─────────────────────────────────────────────────────────────────────────────
...
// ── CONFIG ────────────────────────────────────────────────────────────────────
const ACCESS_PIN = "1234"; // ← Change this to your PIN
...
const DEFAULT_ITEMS = [
  ...
  { id: "d3", category: "ציוד קרגו",  name: "Colored Handwrap - Blue",      unitsPerPack: 4,    orderQty: 0, costPerUnit: 23.00, modelNumber: "S-2900BLU",   received: false, link: "", notes: "" },
  { id: "d4", category: "ציוד קרגו",  name: "Uline Paper Hot Cups 10oz",    unitsPerPack: 1000, orderQty: 0, costPerUnit: 94.00, modelNumber: "S-20105W",    received: false, link: "", notes: "" },
  { id: "d5", category: "ציוד מטבח",  name: "Paper Towels",                 unitsPerPack: 30,   orderQty: 0, costPerUnit: 42.00, modelNumber: "S-7711",      received: false, link: "", notes: "" },
  { id: "d6", category: "ציוד משרדי", name: "BIC Gel Pen - Medium Blue",    unitsPerPack: 1,    orderQty: 0, costPerUnit: 1.30,  modelNumber: "S-21758BLU",  received: false, link: "", notes: "" },
];

const uid = () => Math.random().toString(36).slice(2, 9);

// ── STORAGE (works in Claude artifact AND in browser after Vercel deploy) ─────
const store = {
  get: async (key) => {
    ...
    if (typeof window !== "undefined" && window.storage && typeof window.storage.get === "function") {
      try {
        ...
        // key not found in artifact storage, fall through to localStorage
      }
    }
...
// ── EXCEL IMPORT ──────────────────────────────────────────────────────────────
// Finds a column by trying multiple possible names (Hebrew + English)
...
function parseExcelRows(rows) {
  ...
  if (!nameKey) return []; // can't map without a name column

  const cleanNum = (v) => parseFloat(String(v).replace(/[$,\s]/g, "")) || 0;
  const cleanBool = (v) => {
    const s = String(v).toLowerCase().trim();
    return s === "yes" || s === "כן" || s === "true" || s === "1" || s === "v" || s === "✓";
  };
...
const S = {
  input: {
    ...
    color: "#e4e4e7", borderRadius: 6, padding: "8px 10px", fontSize: 13,
    outline: "none", boxSizing: "border-box", fontFamily: "inherit",
  },
  ghost: {
    background: "transparent", border: "1px solid #3f3f46", color: "#a1a1aa",
    borderRadius: 7, padding: "7px 14px", fontSize: 13, cursor: "pointer", fontFamily: "inherit",
  },
  primary: {
    background: "#f59e0b", border: "none", color: "#000",
    borderRadius: 6, padding: "9px 0", fontSize: 13, fontWeight: 700,
    cursor: "pointer", fontFamily: "inherit", flex: 2,
  },
  cancel: {
  ...
  icon: {
    ...
    fontSize: 14, padding: "2px 5px", borderRadius: 3, transition: "color 0.15s",
  },
  stepBtn: {
    width: 20, height: 20, borderRadius: 4, background: "#27272a",
    border: "1px solid #3f3f46", color: "#a1a1aa", cursor: "pointer",
    ...
    <div style={{ minHeight:"100vh", background:"#09090b", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:20, direction: t.dir }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:10 }}>
        ...
        <button onClick={submit}
          style={{ width:200, background:"#f59e0b", border:"none", color:"#000", borderRadius:8, padding:"10px 0", fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>
          {t.unlockBtn}
        </button>
      </div>
    </div>
...
function ItemModal({ t, item, existingCats, onSave, onClose }) {
  ...
  const fieldFocus = e => { e.target.style.borderColor = "#f59e0b"; };
  const fieldBlur  = e => { e.target.style.borderColor = "#3f3f46"; };
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.78)", zIndex:50, display:"flex", alignItems:"center", justifyContent:"center" }} onClick={onClose}>
      <div style={{ background:"#18181b", border:"1px solid #3f3f46", borderRadius:14, padding:28, width:420, maxHeight:"85vh", overflowY:"auto", boxShadow:"0 24px 64px rgba(0,0,0,0.6)", direction: t.dir }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ marginBottom:14 }}>
          ...
          {customCat ? (
            <input value={form.category} autoFocus style={{ ...S.input, direction: t.dir }}
              onChange={e => set("category", e.target.value)}
              onFocus={fieldFocus} onBlur={e => { fieldBlur(e); if (!form.category) setCustomCat(false); }}
            />
          ) : (
            <select value={form.category} style={{ ...S.input, direction: t.dir, cursor:"pointer" }}
              onChange={e => { if (e.target.value === "__new__") { set("category",""); setCustomCat(true); } else set("category", e.target.value); }}
        ...
        {fields.map(f => (
          <div key={f.key} style={{ marginBottom:14 }}>
            ...
            <input
              ...
              style={{ ...S.input, direction: (f.key === "link" || f.key === "modelNumber") ? "ltr" : t.dir }}
              onFocus={fieldFocus} onBlur={fieldBlur}
            />
          </div>
        ))}
...
function SendFlow({ t, orderItems, template, onClose }) {
  const [step, setStep] = useState("input"); // input | preview | sending | sent | error
  ...
  const buildBody = () => {
    ...
    // Replace placeholders — note: no JS template literals here to avoid conflicts
    return template
      .replace("{{date}}", date)
      .replace("{{items}}", itemLines)
      .replace("{{totalCost}}", orderTotal);
  };
  ...
  const overlay = (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.82)", zIndex:50, display:"flex", alignItems:"center", justifyContent:"center" }}
      onClick={step === "sending" ? undefined : onClose}>
      <div style={{ background:"#18181b", border:"1px solid #3f3f46", borderRadius:14, padding:28, width:480, maxHeight:"85vh", overflowY:"auto", boxShadow:"0 24px 64px rgba(0,0,0,0.6)", direction: t.dir }}
        ...
        {step === "input" && (
          <>
            ...
            {/* To */}
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:11, color:"#71717a", marginBottom:5 }}>{t.recipientLabel}</div>
              <input type="email" value={to} placeholder="email@company.com" onChange={e => setTo(e.target.value)}
            ...
            {/* Subject */}
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:11, color:"#71717a", marginBottom:5 }}>{t.subjectLabel}</div>
              <input type="text" value={subject} onChange={e => setSubject(e.target.value)}
                style={{ ...S.input, direction: t.dir }}
                onFocus={e => { e.target.style.borderColor="#f59e0b"; }}
                onBlur={e => { e.target.style.borderColor="#3f3f46"; }}
              />
            </div>
            ...
            <div style={{ display:"flex", gap:8, marginTop:20 }}>
              ...
              <button
                ...
                onClick={() => { setPreview({ to, subject, body: buildBody() }); setStep("preview"); }}>
                {t.previewBtn}
              </button>
            </div>
          </>
...
export default function App() {
  ...
  useEffect(() => {
    try {
      if (sessionStorage.getItem("cargo-unlocked") === "1") setUnlocked(true);
    } catch {}
  }, []);
  ...
  useEffect(() => {
    if (!unlocked) return;
    (async () => {
      const savedItems = await store.get("cargo-items-v5");
      const savedTpl   = await store.get("cargo-tpl-v3");
      const savedLang  = await store.get("cargo-lang-v1");

      const loadedLang = (savedLang === "he" || savedLang === "en") ? savedLang : "en";
      setLang(loadedLang);
      setItems(savedItems ? JSON.parse(savedItems) : DEFAULT_ITEMS);
      setTemplate(savedTpl || T[loadedLang].defaultTemplate);
    })();
  }, [unlocked]);

  // ── Persist helpers ──────────────────────────────────────────────────────
  const saveItems = async (next) => { await store.set("cargo-items-v5", JSON.stringify(next)); };
  const setAndSave = (next) => { setItems(next); saveItems(next); };
  ...
  const switchLang = (l) => {
    setLang(l);
    store.set("cargo-lang-v1", l);
    // Reset template to new language default only if it's still the old default
    if (template === T["en"].defaultTemplate || template === T["he"].defaultTemplate) {
      setTemplate(T[l].defaultTemplate);
      store.set("cargo-tpl-v3", T[l].defaultTemplate);
    }
  };
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (parsed.length > 0) {
          ...
          alert("No items found. Make sure the Excel has a column named 'מוצר' or 'Item Name'.");
        }
      } catch (err) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#09090b", color:"#e4e4e7", fontFamily:"'Heebo','Segoe UI',sans-serif", direction: t.dir }}>
      ...
      <div style={{ display:"flex", gap:4, padding:"10px 24px", borderBottom:"1px solid #1c1c1f", overflowX:"auto" }}>
        {[{ id:"__all__", label: t.catAll, count: items.length }, ...allCats.map(c => ({ id:c, label: t.catNames[c] || c, count: items.filter(i => i.category === c).length }))].map(cat => (
          <button key={cat.id} onClick={() => setActiveCat(cat.id)}
            style={{ padding:"5px 14px", borderRadius:99, border:"1px solid", fontSize:12, fontWeight:600, cursor:"pointer", whiteSpace:"nowrap", fontFamily:"inherit", transition:"all .15s",
              background: activeCat === cat.id ? "#f59e0b" : "transparent",
              color:       activeCat === cat.id ? "#000" : "#71717a",
              borderColor: activeCat === cat.id ? "#f59e0b" : "#3f3f46",
            }}>
            {cat.label}
            <span style={{ marginLeft: t.dir === "ltr" ? 5 : 0, marginRight: t.dir === "rtl" ? 5 : 0, opacity:0.6, fontSize:10 }}>({cat.count})</span>
          </button>
        ))}
      </div>
      ...
      <div>
        {filtered.length === 0 && (
          <div style={{ textAlign:"center", padding:48, color:"#3f3f46", fontSize:13 }}>{t.noItems}</div>
        )}
        {filtered.map(item => (
          <div key={item.id}
          ...
          >
            ...
            <div style={{ display:"flex", alignItems:"center", gap:7, overflow:"hidden" }}>
              {item.orderQty > 0 && !item.received && (
                <span style={{ width:5, height:5, borderRadius:"50%", background:"#f59e0b", flexShrink:0, boxShadow:"0 0 5px #f59e0b88" }} />
              )}
              <span
                title={item.name}
            ...
            <div style={{ display:"flex", justifyContent:"center" }}>
              <button title={item.received ? t.rcvdTitle : ""}
                onClick={() => patchItem(item.id, { received: !item.received })}
            ...
            <div style={{ display:"flex", gap:3, justifyContent:"center" }}>
              <button style={{ ...S.icon, color:"#52525b" }} title="Edit"
                onMouseEnter={e => { e.currentTarget.style.color="#e4e4e7"; }}
                onMouseLeave={e => { e.currentTarget.style.color="#52525b"; }}
                onClick={() => setEditItem(item)}>✎</button>
              <button style={{ ...S.icon, color:"#3f3f46" }} title="Delete"
      ...
      {/* ── MODALS ── */}
      {showAdd && (
        <ItemModal t={t} existingCats={existingCats} onSave={upsertItem} onClose={() => setShowAdd(false)} />
      )}
      {editItem && (
        <ItemModal t={t} item={editItem} existingCats={existingCats} onSave={upsertItem} onClose={() => setEditItem(null)} />
      )}
      {showTpl && (
        <TemplateEditor t={t} template={template}
          onSave={tpl => { setTemplate(tpl); store.set("cargo-tpl-v3", tpl); }}
          onClose={() => setShowTpl(false)}
        />
      )}
      {showEmail && (
        <SendFlow t={t} orderItems={orderItems} template={template} onClose={() => setShowEmail(false)} />
      )}
    </div>
  );
}